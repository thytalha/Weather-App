...
const AQI_INFO = [
    null,
    { label: 'Good',                         color: '#22c55e', range: '0–50',   desc: 'Air quality is satisfactory.' },
    { label: 'Moderate',                     color: '#eab308', range: '51–100', desc: 'Acceptable quality; some pollutants may concern sensitive groups.' },
    { label: 'Unhealthy for Sensitive',      color: '#f97316', range: '101–150',desc: 'Sensitive groups may experience health effects.' },
    { label: 'Unhealthy',                    color: '#ef4444', range: '151–200',desc: 'Everyone may begin to experience health effects.' },
    { label: 'Very Unhealthy',               color: '#a855f7', range: '201–300',desc: 'Health alert: everyone may experience serious effects.' },
    { label: 'Hazardous',                    color: '#dc2626', range: '301+',   desc: 'Health warning of emergency conditions.' },
];
        ...
        if (this.type === 'thunder' && this.lightningFlash > 0) {
            ...
            if (this.lightningFlash > 5) {
                this.drawLightningBolt(ctx,
                    W * 0.3 + Math.random() * W * 0.4, 0,
                    W * 0.2 + Math.random() * W * 0.6, H * 0.55);
            }
        }
...
class TempChart {
    constructor() {
        ...
        this.canvas.addEventListener('touchstart', this._onTouch.bind(this), { passive: true });
        this.canvas.addEventListener('touchmove',  this._onTouch.bind(this), { passive: true });
    ...
    render(progress = 1) {
        ...
        const getY = tmp => P.top  + chartH - ((tmp - minTemp) / (maxTemp - minTemp)) * chartH;
        ...
        this.data.forEach((d, i) => {
            ...
            const x = getX(i);
            if (x > clipX + 2) return;
            const label = d.time.toLocaleTimeString([], { hour: 'numeric', hour12: true });
    ...
    _getIndexAtX(clientX) {
        ...
        const idx    = Math.round((x - this.PAD.left) / (chartW / (this.data.length - 1)));
    ...
    _showTooltip(clientX, clientY, index) {
        ...
        const timeStr = d.time.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit', hour12: true });
        ...
        if (left + 130 > containerRect.width) left = clientX - containerRect.left - 130;
        if (top < 4) top = 4;
...
function toSpeed(kph){ return currentSpeedUnit === 'mph' ? Math.round(kph * 0.621371) : Math.round(kph); }
...
cityInput.addEventListener('keydown', e => {
    if (e.key === 'Enter')  { hideSuggestions(); fetchCoordinates(cityInput.value.trim()); }
...
function renderAQI(aqiData) {
    if (!aqiData) { aqiSection.classList.add('hidden'); return; }
    ...
    const pollutantHTML = pollutants.map(p => `
    ...
    requestAnimationFrame(() => {
        requestAnimationFrame(() => {
            ...
            if (fillEl) {
                fillEl.style.transition = 'stroke-dasharray 1.4s cubic-bezier(0.4,0,0.2,1)';
                fillEl.setAttribute('stroke-dasharray', `${fillArc} ${AQI_CIRCUM - fillArc}`);
            }
...
function renderAlerts(alertsObj) {
    ...
    if (allAlerts.length > 1) {
        ...
        document.getElementById('alertPrev').addEventListener('click', () => {
            currentAlertIdx = (currentAlertIdx - 1 + allAlerts.length) % allAlerts.length;
...
function showAlert(idx) {
    ...
    document.getElementById('alertDesc').textContent =
        alert.desc ? (alert.desc.length > 160 ? alert.desc.substring(0, 157) + '…' : alert.desc) : '';
...
function getWeatherCondition(code, isDay) {
    ...
    if ([1063, 1150, 1153, 1168, 1171, 1180, 1183,
         1186, 1189, 1192, 1195, 1240, 1243, 1246].includes(code)) return { icon: '🌧️' };
    if ([1198, 1201].includes(code))                   return { icon: '🌨️' };
    if ([1066, 1114, 1117, 1210, 1213, 1216, 1219,
         1222, 1225, 1255, 1258].includes(code))       return { icon: '❄️' };
...
function onDragEnd() {
    ...
    document.querySelectorAll('.metric').forEach(m => m.classList.remove('drag-over', 'dragging'));
    draggedMetric = null;
}

function saveMetricOrder() {
    const order = [...metricsGrid.querySelectorAll('.metric')].map(m => m.dataset.metric);
